...
The HTML pages under site-v2/docs/ were originally produced by hand from this
...
surfaces move together.
    ...
    python3 scripts/generate-docs-site.py [--out site-v2/docs] [--check]
...
def render_markdown(md: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    while i < len(lines):
        ...
        if m:
            ...
            if level == 1:
                out.append(f'<h1 id="{hid}">{inline(text)}{anchor}</h1>')
            else:
                out.append(f'<h{level} id="{hid}">{inline(text)}{anchor}</h{level}>')
                if level == 2:
                    toc.append((hid, text))
            i += 1
            continue
...
def build_index(order, group_of) -> str:
    ...
    body: list[str] = [
        ...
        '      <h1 id="overview">DGC documentation<a class="anchor" href="#overview" '
        'aria-label="Link to this section">#</a></h1>',
        "<p>DGC is a local-first coding agent for your terminal and for VS&nbsp;Code. It runs a real "
        "agentic loop — planning, reading files, running tools, editing code — against a model "
        "<strong>you</strong> choose: a local endpoint, any OpenAI-compatible host, or a coding "
        "subscription you already pay for. Your code and your prompts go only where you point them.</p>",
    ...
    for group, titles in GROUPS:
        ...
        body.append(f'<h2 id="{gid}">{html.escape(group)}'
                    f'<a class="anchor" href="#{gid}" aria-label="Link to this section">#</a></h2>')
