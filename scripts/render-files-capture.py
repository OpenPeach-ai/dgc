...
DORMANT — running this publishes output the site no longer declares. FIG.6 is a scripted
illustration now, and the site declares only the ``cli`` and ``editor`` capture sets, so a run
that reaches publish() leaves ``site/assets/files-capture.{webm,mp4}``,
``files-capture-poster.jpg`` and ``assets/files-replay.json`` in the public tree plus a
``captures.files`` entry in ``site-src/data/capture-media.json`` — and check-site.py then fails
both its public-tree gate ("undeclared outputs") and its capture-manifest gate. Kept, unchanged,
for the day the file-explorer capture is published again: re-declaring the set
(CAPTURE_MEDIA_FILES, STATIC_PUBLIC_FILES and the figure partial) has to land with the recording.
