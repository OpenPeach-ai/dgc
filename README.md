...
- **Standing goals** — `/goal <objective>` keeps a bounded local objective record across turns and resume; native local/API turns also receive it in their instructions. `/goal complete`, `/goal blocked`, `/goal resume`, and `/goal clear` make its lifecycle explicit; the CLI, editor backend, and ACP adapter carry typed goal state. Delegated vendor turns do not receive that DGC goal instruction.
