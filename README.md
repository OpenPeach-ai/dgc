...
- **MCP servers** — connect stdio MCP servers (configured in `~/.dgc/config.json` → `mcp_servers`) and their tools join DGC's own. DGC negotiates the stateless MCP 2026 wire with a fresh-process fallback for handshake-era servers, carries roots MRTR, cancellation, progress, severity-filtered logs, typed content, and visible process failures; `/mcp` shows each server's negotiated era.
