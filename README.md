...
- **Adaptive tool and skill catalog** — ordinary coding turns keep the complete core read/edit/search/shell surface while web, artifact, skill-install, memory, goal, delegation, and reusable-skill metadata appear only when the request needs them. `bash_output`/`bash_kill` appear automatically only while that agent owns retained output/a background process, and full-auto omits the blocking options prompt. This removes repeated irrelevant schema/instruction prefill for small local models; set `tool_profile: full` to expose the full catalog allowed by the current permission mode.
