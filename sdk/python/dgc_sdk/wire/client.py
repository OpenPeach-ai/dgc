...
class DGCClient:
    ...
    def request(
        ...
        *,
        ...
        ``uncorrelated_reply`` also accepts a ``response_type`` event without a ``request_id``,
        for commands the backend acknowledges with a broadcast (``clear_todos`` -> ``todos``).
        """
        ...
        def answers(event: Mapping[str, Any]) -> bool:
            ...
            if reply_id is not None:
                return False
            if uncorrelated_reply and kind == response_type:
                return True
