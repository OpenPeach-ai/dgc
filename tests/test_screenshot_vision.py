...
class _UI:
    ...
    def tool_images(self, call_id, images, caption=""):
        self.images.append((call_id, list(images), caption))
...
class ScreenshotVisionTests(unittest.TestCase):
    ...
    def test_a_fresh_agent_carries_its_clients_image_capability(self):
        ...
        self.assertTrue(agent.ctx.vision, "a new backend must not tell a vision model it is blind")
    ...
    def test_each_screenshot_in_a_batch_is_labelled_for_the_model_and_the_panel(self):
        ...
        def fake_browser(args, ctx):
            page = pages[args["url"]]
            tools._queue_image(ctx.tool_owner, f"data:image/png;base64,{args['url']}",
        ...
        captions = [caption for _, _, caption in self.ui.images]
        self.assertEqual(captions, [f"browser screenshot of {pages['a']}",
                                    f"browser screenshot of {pages['b']}"])
    ...
    def test_a_screenshot_with_a_url_opens_that_page_first(self):
        ...
        self.assertEqual([label for _, label in queued], ["screenshot of http://127.0.0.1:4901/page1.html"])
