...
class AgentStallTests(StallTestCase):
    ...
    def test_a_hook_ui_mid_stream_stall_is_a_continuation_run_with_a_tagged_notice(self):
        ...
        # The continuation is joined onto the partial answer, which keeps the recovery prompt's notice.
        self.assertFalse([m for m in agent.messages if m.get("_dgc_notice")])
        notices = [n for m in agent.messages for n in (m.get("_dgc_stream_recoveries") or [])]
