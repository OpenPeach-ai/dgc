...
profile_for() { # $1 = deny|allow  -> prints line1=exec line2=profile
  DGC_SBX_NET="$1" DGC_SBX_WORK="$W" PYTHONPATH="$REPO_ROOT" "$PY" - <<'PYEOF'
import os, sys
try:
...
run_probes_for_mode() {
  ...
  probe() { # name  expect-substr(""=any)  command...
    ...
    o=$("$exec_path" -p "$profile" /bin/bash -o pipefail -c "$*" 2>&1); code=$?
    ...
    if [ "$verdict" = LEAK ]; then
      ...
      echo "BLOCKED [$net] $name  (exit=$code)"
    fi
  }
  ...
  if "$exec_path" -p "$profile" /bin/bash -o pipefail -c "echo ok > '$W/written.txt'" 2>/dev/null \
