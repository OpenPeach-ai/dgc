...
def _image_entry(data: bytes, *, name: str, source: str, host: str = "", path: str = "",
                 label: str = "") -> dict:
    """``label`` is what the model reads before the image (a screenshot names its page); the chat
    shows ``name`` and ``host`` instead, never the page URL."""
    ...
    return {"data": bytes(data), "mime": image_views.sniff(data) or "image/png",
            ...
            "bytes": len(data), "label": str(label or "")}
...
def browser_tool(args: dict, ctx) -> str:
    ...
    try:
        ...
        if operation == "screenshot":
            ...
            # only the model's label names the page.
            ...
            if _vision_available(ctx):
                ...
                seen = ("This model does not accept images, so you cannot look at it — use "
                        ...
                        + (". The user can see the screenshot in the chat." if _shows_images(ctx) else
                           ", and tell the user the file path so they can open it."))
...
def python(args: dict, ctx) -> str:
    # No sandbox check here on purpose. The OS sandbox confines the SHELL and never this
    # interpreter, which is the real hazard -- but WHO decides about it belongs to the permission
    # layer, not to the executor. `permissions.session_policy_rules` already denies python in auto
    # mode (nobody is there to review) and leaves every other mode a permission request, which is
    # the contract an SDK host is given and answers through `on_permission`. An unconditional
    # refusal here overrode that host, in every mode, with no way to switch it off.
...
def _view_image_bytes(p: Path, data: bytes, ctx, *, source: str) -> str:
    """Queue a workspace image file for this call (the chat shows it; a model that can see gets it
    after the batch) and say what was viewed. ``data`` is a sniffed image of at most 8 MB."""
    mime = image_views.sniff(data) or "image/png"
    try:
        rel = p.relative_to(Path(ctx.project_root).resolve(strict=False)).as_posix()
    except ValueError:
        rel = str(p)
    entry = _image_entry(data, name=p.name, source=source, path=str(p))
