...
BUILTIN_COMMANDS: tuple[CommandSpec, ...] = (
    ...
    CommandSpec("connect", "pick a subscription, a provider, or a custom host", _TCE, "connect",
                usage="connect [ENGINE|PRESET|URL]"),
    ...
    CommandSpec("think", "how hard the model reasons: off · low · medium · high · xhigh", _TCE,
                "pickThink", usage="think [LEVEL]", available_while_running=True),
    ...
    CommandSpec("preserve-thinking", "keep prior-turn reasoning in context (compatible hosts)",
                _TCE, "togglePreserveThinking", usage="preserve-thinking [on|off]",
                aliases=("preserve-reasoning",)),
    ...
    CommandSpec("monitors", "list, stop, or pause wake-ups from background monitors",
                frozenset({"tui", "editor"}), "monitors", accepts_args=True,
                usage="monitors [stop ID|all · wake on|off · show ID]", available_while_running=True),
    CommandSpec("copy", "copy the last reply to your clipboard (all · code · N)", _T,
                accepts_args=True, usage="copy [all|code|N]", available_while_running=True),
    CommandSpec("select", "drag-select mode — releases the mouse to your terminal", _T,
                aliases=("selection",), available_while_running=True),
    CommandSpec("mouse", "remember whether DGC captures the mouse (on · off)", _T,
                accepts_args=True, usage="mouse [on|off]", available_while_running=True),
    CommandSpec("export", "save this conversation as Markdown", _T,
                accepts_args=True, usage="export [FILE]"),
    ...
    CommandSpec("sandbox", "confine bash with the supported host OS boundary",
                frozenset({"tui", "editor"}), "securitySettings"),
    CommandSpec("bg", "terminal background: auto · dark · light · inherit", _TC,
                aliases=("background",), usage="bg [auto|dark|light|inherit]"),
    CommandSpec("theme", "colour theme: auto · dark · light", _TC, usage="theme [NAME]"),
    CommandSpec("context", "context-window usage", frozenset({"tui", "classic", "editor"}),
                "status"),
    CommandSpec("usage", "tokens counted on this machine, by model and day", _TCE, "usage",
                accepts_args=True, usage="usage [today|7d|30d|month|all]",
    ...
    CommandSpec("handoff", "generate a complete continuation handoff", _TCE, "handoff",
                aliases=("handover",)),
    CommandSpec("export-training", "export this project's sessions as scrubbed fine-tuning JSONL",
                _TC, usage="export-training [FILE]", aliases=("export-jsonl",)),
    ...
    CommandSpec("memory", "show or add project/user memory",
                frozenset({"tui", "classic", "editor"}), "memory", True,
                usage="memory [ACTION…]"),
    CommandSpec("permissions", "list or add allow · ask · deny rules",
                frozenset({"tui", "classic", "editor"}), "permissions",
                usage="permissions [RULE…]"),
    CommandSpec("notes", "what this project already learned · search it", _TC,
                accepts_args=True, usage="notes [QUERY|on|off]", available_while_running=True),
