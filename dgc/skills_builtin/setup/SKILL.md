...
3. Add resilience: in `~/.dgc/config.json` set `fallback_model` (+ `fallback_base_url` if on another host). For sub-agents on a different host/model, set `subagent_model`/`subagent_base_url` globally and provide the credential through `DGC_SUBAGENT_API_KEY`, or pin per-agent overrides in `.dgc/agents/*.md` with `api_key_env: MY_PROVIDER_KEY`.
