...
## (C) FEWER PROMPTS
7. Find the safe, repetitive calls that keep prompting (read-only bash, idempotent build/test): `git status/diff/log/show/branch`, `ls`, `cat`, `grep`/`rg`, `find`, `npm run test/lint/build`, `pytest`. For each, propose a tightly-scoped rule scoped to the subcommand: `/permissions allow Bash(git status:*)`, `Bash(git diff:*)`, `Bash(npm run test:*)`, `Bash(rg:*)`. Present the list with one line of why each is safe; let the user apply it.
