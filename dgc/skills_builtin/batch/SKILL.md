...
3. For independent units, use `task` when delegation is available and authorized. Give each worker a bounded, self-contained assignment, owned files, acceptance criteria and a relevant existing example. Respect the configured concurrency and permission mode; do not enable full-auto merely to obtain parallel execution. If delegation is unavailable, process the units sequentially using the same ownership boundaries.
