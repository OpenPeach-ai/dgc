...
| Prove current Python/runtime behavior | `tests/run_tests.py`: **1053/1053**; `compileall` and source `git diff --check` pass | Proven locally |
...
11. Compatible Chat JSON and tool streams now normalize spec fragments, repeated complete IDs/names,
    ...
    are bounded; malformed events, post-finish data, missing terminal state, and unfinished calls fail
    before execution; cancellation drops partial calls; content-filter/unknown stops enter the existing
    non-executable reissue path. Explicit `finish_reason`-then-EOF remains supported for local gateways,
    ...
    `_unparsed` repair result and are never executed as a malformed type. Twenty-two wire checks cover these
    variants without contacting a provider.
