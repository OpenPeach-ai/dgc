...
| Extension tests | 2 / 2 | 22 / 22 + host 1 / 1 | jsdom protocol/render/safety/accessibility, real spawned-child transport/backpressure/decision/state-correlation flows, and activation/command registration/exact webview handshake plus permission/plan lifecycles inside installed VS Code are green. |
...
launch, protocol-v3 handshake/shape/sequence validation, byte- and count-bounded retention, preserved
...
17. The protocol-v3 headless surface now exposes bounded, correlated `list_mcp_tools` pagination and
...
22. The generated protocol-v3 JSON Schema is now an installed-package artifact rather than only a
