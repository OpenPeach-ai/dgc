...
The complete offline evidence is 835/835 Python checks, 19/19 editor transport/webview checks, and
...
evidence is 835/835 Python checks, 19/19 editor transport/webview checks, and 1/1 installed-VS-Code
host smoke.
...
14. Context budgeting now measures the provider-visible transcript plus the exact state-aware native
    ...
    real compaction threshold instead of remaining hidden until a provider overflow. Catalogs whose
    definitions alone exceed a model window still require lazy/context-proportional exposure rather
    than more compaction.
