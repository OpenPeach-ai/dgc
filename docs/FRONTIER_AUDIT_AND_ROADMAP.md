...
|---|---:|---:|---|
| Python test harness | 225 / 226 | 427 / 427 | Environment-independent unit, adversarial, interaction-contract, protocol, benchmark-control and mock-model E2E coverage. |
...
| Full controlled league | None | New replacement run required | Attempts 1–3 exposed compaction-sensitive counters, hard-timeout persistence loss, deleted isolated-grader paths, and a 573-second late provider completion. A clean targeted attempt 4 on `cpp/binary-search-tree` validated portable round-two paths and the fail-closed quiescence boundary: recovery improved from compile failure to 10/12 tests, but trace evidence exposed four retry-generated provider completions after deadline cancellation and a premature five-failure stop despite landed edits. All attempts are diagnostic only. DGC now treats deadline cancellation as terminal before retry, charges and explicitly reconciles provider-only cancelled work, resets varied-command failure streaks on landed edits while retaining identical-failure evidence, and preserves the earlier path/accounting/closeout controls. These cases are covered by 427 checks; the hard case and then the complete league require a new clean candidate run. |
