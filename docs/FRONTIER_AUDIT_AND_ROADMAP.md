...
| Prove current Python/runtime behavior | `tests/run_tests.py`: **1038/1038**; `compileall` and source `git diff --check` pass | Proven locally |
...
10. Chat Completions, Responses, native Anthropic Messages, and native Ollama now share bounded
   ...
   deterministic response ownership. Error bodies, native Ollama JSON/NDJSON, native Anthropic SSE,
   and Responses JSON/SSE are bounded before line or whole-body materialization. Error,
