...
| Runtime correctness | Atomic file/session writes with bounded post-crash temp reclamation, UUID/private schema-v6 sessions with validated monotonic generations, crash-released cross-process session-family leases and compare-and-swap writes, content-addressed exact conversation/file checkpoints durable across resume and compaction with late-symlink-safe capture/restore, fail-closed pre-edit persistence, transactional rewind persistence/rollback, exact bounded transactional last-known-good recovery, atomic lease/CAS memory appends without lost concurrent facts, transactional exact-generation compaction, provider-safe tool-group repair/compaction including final-commit closure after interruption, truthful terminal turn outcomes, split-stream-safe credential redaction across model/tool/wire/durable conversation boundaries, generation-stable bounded/redacted handoffs saved as new private no-follow workspace files, immediate text-tool fallback, full process-group cleanup, bounded redacted lifecycle-hook output, bounded internal-Git diagnostics, and bounded/redacted/cancellable foreground, background, and direct-terminal command output with session-scoped continuation handles | Optional at-rest encryption for exact rewind file snapshots and broader cross-platform crash-fuzz campaigns |
| Concurrency | Per-session ACP/headless runtimes, truthful busy/error completion, race-free count-and-byte-bounded editor follow-up FIFO across completion/cancel boundaries, atomic TUI steer-or-defer ownership with a bounded per-session next-turn FIFO across model closeout/cancel/failure and direct-shell execution, startup-safe cancellation in editor/ACP/classic/TUI workers, owner-private crash-safe cross-process checkout/session-family/full-turn leases, generation preflight before hooks or model execution, revision/existence guards that reject stale transcript/metrics/goal/name/plan/workspace/delete mutations, active-turn exclusion for deletion/rewind/compaction/handoff/retained work and TUI workspace attach/finalize, pre-edit snapshots and rewind captured/restored inside the checkout lease, background leases held to process exit, lifecycle hooks serialized under the same checkout lease with one cancellable batch deadline, foreground protocol terminal events emitted only after their reusable worker slot is released, separate TUI config/MCP state, automatic source-leased TUI fleet worktrees whose internal Git is pinned outside the repository, non-interactive, hook-suppressed, output-bounded, and process-group reaped, exact dirty baselines and safe resume/retention, manual named worktrees, automatic `task` worktrees, bounded concurrent all-task batches, deterministic conflict-safe integration, plus typed terminal/editor retained-task inspect/apply/drop recovery with parent rewind | Cross-platform fleet crash/reopen soak and measured local/remote fan-out latency/throughput |
| Model effectiveness | Typed provider profiles and endpoint+model capability negotiation; native Ollama chat/model discovery with exact thinking/tool continuation, options and usage plus bounded JSON/NDJSON, terminal-`done` attestation, bounded call accumulation, and cancellation-safe partial-state discard; OpenAI-compatible Chat Completions with bounded JSON/SSE, explicit terminal lifecycle, cancellation-safe partial-call discard, safe stop-reason handling, normalized hostile usage, current `tool_calls` plus deprecated `function_call` support, and cumulative/missing-index local-gateway normalization; native Anthropic Messages with provider authentication, fail-closed signed/encrypted state, exact bounded `pause_turn` replay, grouped client/server tool state, citations, adaptive/legacy thinking, terminal lifecycle validation, cache usage, bounded selected-model input/output metadata, vision and bounded output negotiation; OpenAI Responses with opt-in stored continuation, default stateless encrypted-reasoning replay, prompt-cache routing, nested usage accounting, and bounded cancellation-safe `/responses/compact` continuation with exact opaque replay, usage-derived context accounting, durable resume, capability fallback, and deterministic local evidence fallback; all four transports distinguish cancellation, malformed protocol, explicit terminal completion, and recoverable nonterminal EOF/reset: partial calls never execute, incomplete provider state is never replayed, valid terminal state survives a later socket reset, and recovery shares a three-request turn bound without adding a request to normal completions; cancellation-safe bounded retry/backoff and streamed-response cleanup across all transports; independently scoped primary/fallback/sub-agent transports and credentials; provider-wire context budgeting that includes exact state-aware native tool schemas and transcripts, Responses conversion, and bounded dimension-based vision estimates instead of base64 transport bytes; provider limits clamp impossible operating windows without silently expanding local KV-cache allocation; day-stable system prefixes and compact text-tool catalogs for prefix/prefill efficiency; idle-only serialized/cancelable title and suggestion generation; stable call IDs, hash-addressed atomic `apply_patch`, repository map, bounded static code intelligence plus explicitly configured managed LSP symbols/diagnostics/definitions/references with capped per-project reuse, state/intent-aware tool exposure including navigation schemas, parallel independent reads, lean plan-mode tools, stronger convergence guards, and bounded verified-final publication that withholds rejected completion claims without losing commentary-before-tool cadence | Additional native transports, native compaction for non-OpenAI providers, richer compatible-provider discovery, optional tree-sitter parsing and measured code-intelligence accuracy/latency |
| MCP / ACP | Dual-era stdio MCP negotiation: real stateless 2026 discovery/per-request metadata with fresh-process legacy fallback, deterministic resource-bounded tool catalogs, validated TTL/scope caching, generation-safe ID-correlated subscriptions plus legacy invalidation, and roots/elicitation/sampling MRTR; small catalogs remain direct while oversized catalogs use a bounded generation-scoped lexical index, deterministic inflection-aware selection, bounded untrusted `mcp_search`, next-request direct-schema activation, and approval-gated `mcp_call` fallback within a context-proportional budget; frontend-specific capability negotiation, credential-safe validated forms, consent-gated URL navigation, twice-approved tools/context-free sampling, associated legacy callbacks, typed content/resources, correlated progress, severity-filtered logging, cancellation, visible failures and process cleanup; typed headless catalog pagination, exact-route invocation, and live server management reuse the full permission/hook/lease/input/redaction path while preserving a responsive approval/cancel channel; safe MCP metadata persists separately from process-local environment values and remote bearer credentials; stable ACP v1 multi-session operations, plan approval, roots and stdio MCP with bounded UTF-8 frames, prompt blocks/text/images, signature-validated data-only images, and boundary-safe untrusted resource framing; wheel-bundled protocol-v4 JSON Schema plus side-effect-free installed-contract discovery/validation and a bounded stdlib synchronous Python client | Real-world large-catalog retrieval accuracy/latency, wider external MCP/ACP conformance, durable/shared cache policy if measurements justify it, and externally published/versioned client packages for other ecosystems |
| Editor | Adapter-backed authenticated model discovery (including native Ollama tags), endpoint-scoped SecretStorage with plaintext-setting removal and stale-key invalidation, installed-host migration/backend-restart/endpoint-invalidation evidence, typed selection/tab/diagnostic resources and canonical multi-root file mentions with display/path separation, acknowledged live multi-root grant reconciliation (including active-turn deferral and removal), accurate failures/IDs/usage/reset/plan feedback, dedicated searchable Skills/Docs/MCP/Permissions/Memory/Hooks surfaces, typed skill detail/reload and command-menu routing without transcript pollution, SecretStorage-backed local/remote MCP management, categorized behavior/security/extension settings, plan/artifact/goal/handoff/retained-task/session parity commands, modal auto warning, a single-source generated protocol-v4 Python/TypeScript/JSON contract, exact-wire validation with optional-field normalization, bounded startup/backpressure queues with priority decision/cancel frames, count/byte-bounded image paste plus backend signature validation, first-response-wins decision and negotiated state/query correlation, stale/restart rejection, strict event shape/sequence validation, restart-on-next-command recovery, streaming-safe Markdown tables and opaque fenced code with exact raw-source copy, real installed-VS-Code activation/command registration/webview-handshake, exact workspace-ack barrier, live multi-root lifecycle, and permission/plan decision evidence, plus automated keyboard/ARIA/reduced-motion/forced-colors coverage, normal-text contrast gates, and non-color tool states | Cross-platform OS-keychain relaunch plus manual screen-reader, 200% zoom, and real Windows High Contrast visual confirmation |
| Evaluation | Pinned six-harness toolchain; engine-scoped schema-v3 records; executable/toolchain provenance; bounded redacted traces; per-exercise HOME; real round-two session continuation; isolated grading; all 225 canonical references validated; transport-normalized reasoning; a baked model-alias context verified before scoring and re-pinned on native requests; synchronized provider-side usage and overlap-aware latency; crash-safe argument-free DGC built-in timing by bounded tool name plus fixed controller-owned completed-request reasons; endpoint-free reproducible lease/filesystem/journal/shell/sandbox overhead probe; per-task request burden, output per generation, outside-provider wall-time attribution, trace-free comparison schema-v5 task rows, exact-task paired baseline deltas with explicit coverage, and bounded per-engine/paired regression reporting; strict clean/full-corpus publication gate | Complete 225-task same-model DGC/Codex/OpenCode/Goose/Pi/Aider league on controlled hardware, then optimize from attributed traces |
...
This audit evaluates the original company objective against the current local candidate; it does not treat a
...
| Prove current Python/runtime behavior | `tests/run_tests.py`: **1085/1085**; focused provider/lifecycle matrix: **132/132**; `compileall` and source `git diff --check` pass | Proven locally |
...
| Prove VS Code/Cursor implementation | Editor tests: **23/23**; TypeScript check and development bundle pass; the installed VS Code host smoke covers activation plus every manifest command, an exact correlated handshake barrier, live multi-root, SecretStorage, permission, and plan lifecycles | Automated local evidence complete; manual/cross-platform evidence remains |
...
Therefore additional unmeasured feature expansion cannot prove leadership; local work remains
defect-driven while the independent matrices run. The company objective itself remains incomplete
until competitive, cross-platform/manual, and release-governance evidence clears the rows above.
...
- After a mutation, a configured verifier owns the final-answer publication boundary. Candidate text
  stays bounded and private until the verifier passes; a failure replaces it in resumable history
  and visibly continues the turn, while tool-call commentary retains its normal ordering.
...
- `# note` and `/memory add [user] TEXT` now behave consistently in both terminal modes. Memory
  appends are bounded, crash-safe, concurrent-writer-safe, and reject link escapes; prompt and UI
  views preserve the file head and newest facts under explicit ceilings.
- `@path` text/image input now behaves consistently in classic, one-shot, and full-screen terminal
  turns. Explicit files use exact no-link reads, byte/count/context ceilings, complete-before-clip
  credential redaction, image magic validation, truthful skip notices, and boundary-safe
  untrusted-data framing. Naming one external file discloses only that file and grants no tool root.
...
|---|---:|---:|---|
| Python test harness | 225 / 226 | 1085 / 1085 | Environment-independent unit, adversarial, interaction-contract, provider, protocol, benchmark-control and mock-model E2E coverage. |
...
| Extension tests | 2 / 2 | 23 / 23 + host 1 / 1 | jsdom protocol/render/safety/accessibility and feature-manager flows, real spawned-child transport/backpressure/decision/state-correlation flows, and activation/all-command registration/exact webview handshake plus permission/plan lifecycles inside installed VS Code are green. |
...
2. **Partly resolved in the current implementation.** Native Ollama and Anthropic now discover
   bounded selected-model metadata before the first generation and cache it by endpoint+model.
   Ollama capabilities select the first-turn protocol; Anthropic input/output limits clamp impossible
   configured values. Explicit overrides and smaller operating windows win, transient failures retain
   compatible behavior, and other providers still rely on family profiles and bounded rejection
   negotiation rather than richer model metadata.
...
the proxy record for audit. Anthropic likewise performs one bounded native
`GET /v1/models/{model_id}` lookup, caches list/detail records without credential material, exposes
the resolved ID and limits diagnostically, clamps configured context/output values downward only,
and treats null/zero/malformed/unavailable metadata as advisory absence. The Qwen3.8 selector uses a
64K local operating recommendation rather than allocating its full 256K trained maximum without
consent. Official OpenAI Responses now uses bounded `/responses/compact` with exact opaque replay,
durable token-accounting metadata, cancellation, capability rejection caching, and local fallback.
Rich compatible-provider discovery and native compaction for other provider families remain open.
...
The complete offline evidence is 914/914 Python checks, 19/19 editor transport/webview checks, and
...
current `wrap(command) -> argv` abstraction cannot establish a truthful Windows security boundary:
an AppContainer launch requires an extended startup attribute list carrying
`PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES`, and a safe writable project needs scoped resource
access rather than an inherited unrestricted user token. The Windows implementation therefore needs
a native launcher/broker that creates or derives an AppContainer profile, applies only the required
capabilities and a credential-minimal environment, grants the canonical workspace access through a
transactional ACL journal, launches the child suspended, assigns it to a kill-on-close Job Object,
and resumes it only after every boundary is installed. ACL revocation and crash recovery must be
idempotent. The design follows Microsoft's
[AppContainer launch contract](https://learn.microsoft.com/en-us/windows/win32/secauthz/implementing-an-appcontainer),
[`CreateAppContainerProfile`](https://learn.microsoft.com/en-us/windows/win32/api/userenv/nf-userenv-createappcontainerprofile),
[restricted-token model](https://learn.microsoft.com/en-us/windows/win32/secauthz/restricted-tokens),
and [Job Object lifecycle](https://learn.microsoft.com/en-us/windows/win32/procthread/job-objects).

The Windows boundary is accepted only when a real Windows integration runner proves all of the
following: ambient user files and credential environment variables are unreadable; writes outside
the canonical workspace and private scratch directory fail; loopback and internet access fail until
the explicit network opt-in is present; grandchildren die on timeout, cancellation, parent crash,
and normal completion; workspace symlink/junction and ACL races fail closed; ACL grants are removed
after success, failure, and forced broker termination; and the ordinary unsandboxed path continues
to pass its platform suite. Until those gates are green, configuration may request sandboxing on
Windows, but execution must continue to fail closed and the UI must say that no backend is available.
...
generation-loop inflation visible independently of model verbosity. Comparison schema v5 now carries
one trace-free diagnostic row per engine/task plus exact-task baseline-minus-peer quality, latency,
generation, token, timeout, tool, and edit deltas. Paired summaries state their synchronized coverage
and first-round/second-round/fail quality wins, while `--top-tasks` prints bounded unions of the
slowest/highest-request tasks per engine and baseline quality/latency/request regressions against
...
The same locked ownership boundary makes later TUI submissions a bounded next turn once closeout has
committed, and hands accepted-but-unconsumed steering back after cancellation or failure. Direct
terminal shell work never pretends to accept model steering; typed text waits in that same FIFO.
Built-in timings contain no
...
Every completed DGC generation is now counted under a fixed private controller reason: initial user
turn, tool result, steering, output continuation, truncated-tool reissue, todo/goal/empty-final gate,
verifier evidence, convergence nudge, transport/context retry, fallback, compaction, title,
suggestion, handoff, MCP sampling, or child-agent work. Unknown internal labels fail into `other`;
requests from older sessions are reconciled as `unattributed`. The metrics-schema-v3 map contains
only bounded labels and monotonic counts, uses the same atomic usage-journal write, survives resume
and SIGKILL, rolls child work into the parent as `subagent`, produces exact round-two deltas, and is
carried through aggregate and comparison JSON plus bounded console summaries. It never retains or
derives a label from prompt, model, error, tool argument, command, path, or result text. The next
league can therefore show whether DGC's remaining requests are useful correction/tool continuations
or controller-induced churn without reconstructing sensitive transcripts.
...
user skills. On the 2026-08-27 GB10 candidate it measured 3,299 system characters (approximately 825
tokens), 4,251 compact schema characters (approximately 1,063 tokens across 9 tools), and 2,197
estimated provider-visible input tokens including the 826-character task prompt. That is a 15.4%
first-request estimate reduction from the immediately preceding 2,596-token candidate without
changing permissions, filesystem checks, sandboxing, editing, shell, or plan-mode navigation. No
bundled skill was active: incidental guard prose such as “do not dismiss a failing test” no longer
injects the debug catalog/schema, while explicit “fix failing tests” and round-two “tests still fail”
requests still do.
...
read, 9.252 ms for an approximately 800-byte atomic write, 4.206 ms for a crash-safe activity-journal
update, 4.315 ms for an ordinary no-op shell, 0.515 ms to construct the sandbox policy, and 33.139 ms
for a bubblewrap-confined no-op shell. The confinement delta is therefore about 29 ms per shell
launch: real and worth attributing, but even ten launches add roughly 0.3 seconds, not the hundreds
...
The same gate now owns publication as well as control flow. Once a turn has mutated the checkout,
provider text for an attempted final is held behind a 512,000-character aggregate ceiling until the
configured verifier accepts it. A failed claim is never emitted through CLI, TUI, headless/editor,
or ACP callbacks and is replaced by an explicit marker in resumable history before correction
continues. Length-limited continuations publish once as a coherent accepted answer. Responses that
contain tool calls are known to be progress rather than finals, so their genuine commentary is
released before the corresponding tool cards and ordinary no-verifier turns retain live streaming.
...
Implementation note for step 4: `tool_profile: adaptive` keeps compact edit/search execution tools
available and retains repository/code-intelligence navigation for ordinary open-scope work. An
explicit narrow-file task can omit those heavyweight schemas and their guidance; explicit navigation
intent restores the matching schema and guidance together, including through trusted mid-turn
steering. Untrusted typed editor context cannot defeat either decision. Read-only plan mode always
retains both navigation tools, and `tool_profile: full` remains the complete-catalog escape hatch.
The stateful `present_plan` and `update_goal` tools remain scoped to valid modes/lifecycles and
configured MCP tools
are never filtered. A benchmark-like turn's serialized built-in schemas now fall from 9,910 to 4,251
bytes (57.1%) before provider framing, while explicit navigation/web/artifact/skill/memory/delegation
requests preserve those capabilities at the point they become relevant.

The environment prefix now includes the day rather than a minute-level clock, so otherwise identical
follow-up turns remain byte-stable throughout that day and can reuse exact provider/local prompt
prefixes. Text-tool fallback serializes its unchanged active schemas as compact JSON. In a
representative ordinary coding turn this removed 1,736 system-prompt characters (about 434 tokens by
DGC's estimator); the catalog itself fell from 7,460 to 5,730 characters (23.2%). Semantic JSON
contracts and the mock-model text-tool E2E remain green; this is deterministic prefill evidence, not
a substitute for league latency measurements.
...
Provider-visible context estimation now uses the selected adapter's exact transcript shape and the
same state-aware tool-schema snapshot as the following request. For validated multimodal messages,
the estimator removes only the base64 transport body from its text approximation and adds a bounded
visual estimate from PNG, JPEG, GIF, WebP, or BMP dimensions. Compression size therefore cannot make
the same image consume fictitious language tokens; a bounded decoded-size fallback covers missing
dimensions, malformed base64 remains in ordinary text accounting, and estimation never mutates the
canonical or outbound provider payload. Deterministic contracts cover Chat Completions, Responses,
and native Ollama, including a megabyte-scale screenshot that now stays below a 4K compaction gate.

Large MCP catalogs now construct exposed schemas in one pass and build a bounded lexical index once
per catalog generation. Search and direct-schema selection reuse that index until a negotiated
refresh atomically replaces routes, schemas, and search data; parameter vocabulary is capped at
8,192 characters, 4,096 nodes, and 1,024 normalized terms per tool. A deterministic 256-tool corpus
retrieves all six target routes for plural/past/progressive user phrasing without selecting filler. On the audit host, an
illustrative 512-tool corpus with 8,000-character parameter descriptions built its index in 59.8 ms;
reused selection averaged 9.1 ms versus 62.2 ms when deliberately forcing a full re-index. This is
local hot-path evidence, not a substitute for real external-server retrieval evaluation.
...
Implementation note for step 3: `dgc/editor_protocol.py` is now the protocol-v4 source of truth and
generates the checked-in review Schema, wheel-bundled Schema, and TypeScript client contract. Python validates every
...
The same generator now places the exact schema in the Python package and packaging metadata includes
it in built wheels. `dgc protocol describe` reports the installed version/hash/limits, required and
optional command/event fields, and canonical TUI/classic/editor slash surfaces without constructing
configuration or touching user/model state. `schema` returns the bundled bytes and `validate`
checks bounded command/event NDJSON with line-correlated, non-reflective diagnostics. The installed
`dgc.client.DGCClient` now provides a one-shot, dependency-free synchronous controller: safe argv
launch, protocol-v4 handshake/shape/sequence validation, byte- and count-bounded retention, preserved
unrelated events during correlation, exact first-response-wins decision IDs, bounded writes and
stderr, and graceful shutdown followed by child/POSIX-process-group reap. Real-backend discovery plus
hostile
wrong-version, duplicate-sequence, non-finite/unknown/oversized-frame, queue-overflow, and stalled-
writer fixtures cover this seam. External publication/versioning and clients for other language
...
The installed host also drives a networkless synthetic turn through the exact message handler used
by the webview, proves exact permission and rejected-plan feedback responses reach the child once, sees
both request-expiry frames and terminal turn state posted back to the live view, and rejects late
replays without restarting or duplicating either decision.
It also injects a mismatched workspace-root acknowledgement before the exact delayed response and
proves a queued prompt remains behind the handshake barrier. Negotiated optional request IDs now
cover editor state/query routes; concurrent replies and rejections settle only their originating
operation, while timeout, fatal transport failure, and child exit release all temporary listeners.
...
removal, tool/reasoning disclosures, live status, reduced-motion CSS, normal-text palette contrast,
system forced-colors, and tool outcomes that remain explicit without color. Decision lifecycles are now
...
spawned-child stdin pressure, jsdom interaction races, and installed-host permission/plan round-trips.
Cross-platform OS-keychain relaunch and manual assistive-technology review remain.
...
compatible. Wider external conformance and published/versioned clients for other language ecosystems
remain open.
...
- The runtime streams model commentary before rendering tool cards, requires a normal final answer,
  ...
  goal convergence guards. A configured verifier owns final publication after mutation, so rejected
  completion claims never appear as accepted terminal/editor/ACP output or resumable history.
...
5. Native Ollama uses `/api/chat` and `/api/tags` directly, preserving streamed thinking, correlated
   tool calls, exact continuation fields, options, keep-alive, cancellation, and provider usage. JSON
   and NDJSON bodies plus native call accumulation are bounded; only a boolean terminal `done: true`
   can finalize executable calls or replay state, post-terminal data fails closed, and cancellation
   discards partial calls. Auto mode falls back safely when a proxy does not expose the native route.
...
9. Local compaction is now deadline-aware and bounded to a 1,024-token, 120-second, reasoning-off
   ...
   earlier context. Official OpenAI Responses first compacts the old group-aligned prefix through
   `/responses/compact`, validates and durably replays its documented user-message-plus-opaque-item
   envelope, preserves a local human-readable resume brief, and uses bounded provider output usage
   rather than ciphertext length for the effective context estimate. Unsupported, malformed,
   oversized, canceled, late, or credential-bearing results fall through to the same local path;
   native compaction for other provider families remains future work.
10. Chat Completions, Responses, native Anthropic Messages, and native Ollama now share bounded
   ...
   deterministic response ownership. Error bodies, Chat Completions JSON/SSE, native Ollama
   JSON/NDJSON, native Anthropic SSE, and Responses JSON/SSE are bounded before line or whole-body
   materialization. Error,
   capability-negotiation, retry, transport-fallback,
   successful-consumption, and parser-failure paths release streamed responses before control moves
   on. JSON fallback reads on every provider are cancellation-aware. A clean EOF, connection reset,
   timeout, chunk-framing failure, or content-decoding interruption before the provider's terminal
   event becomes a distinct `incomplete` result; malformed JSON/events, contradictory lifecycle
   data, safety-bound violations, and caller failures remain immediate errors. An explicit terminal
   event followed by a socket reset remains a successful terminal response. Adversarial tests prove
   a five-second `Retry-After` is interrupted without a second request on all four transports.
11. Compatible Chat JSON and tool streams now normalize spec fragments, repeated complete IDs/names,
    cumulative argument snapshots, numeric-string or missing indices, direct argument objects, and
    deprecated single `function_call` deltas into stable calls. JSON/SSE bodies and accumulated calls
    are bounded; cancellation drops partial calls, malformed or post-terminal events fail immediately,
    and missing terminal state enters the Agent's shared three-request non-executable reissue/continuation
    path. Partial calls never cross the executor boundary, nameless fragments never create an invalid
    native transcript group, and Anthropic/Responses/Ollama opaque continuation state is retained only
    after terminal attestation. Explicit `finish_reason`-then-EOF remains supported for local gateways,
    while the canonical sentinel and final empty-choice usage chunk remain supported. Responses applies
    the same cumulative-argument rule, preserves index zero, and keeps out-of-order calls distinct in
    canonical index order when proxy item IDs are missing. Non-object arguments remain a bounded
    `_unparsed` repair result and are never executed as a malformed type. Provider and Agent regressions
    cover clean EOF, midstream reset, reset after terminal completion, JSON fallback, cancellation,
    malformed frames, repeated interruption bounds, and executor non-entry without contacting a provider.
...
17. The protocol-v4 headless surface now exposes bounded, correlated `list_mcp_tools` pagination and
    exact `call_mcp_tool` execution for already-configured servers. Direct controller calls cannot name
    routes outside the live catalog and run through the Agent security boundary rather than the raw MCP
    manager. A dedicated foreground worker keeps permission/MCP-input/cancel frames responsive, rejects
    overlapping prompts or calls, and emits one correlated terminal status even for denial, cancellation,
    or execution failure. Catalog schemas are reduced to bounded parameter summaries before NDJSON output.
18. Skills and continuation handoffs now have deterministic non-interactive coverage. `list_skills`
    returns only bounded name/description/source metadata and proves whether the loaded definition came
    from the project, user, or bundled precedence layer without leaking a host path. `generate_handoff`
    reserves the session generation across its bounded/redacted auxiliary request and optional save,
    rejects overlapping turns, writes a new owner-private unpredictable `HANDOFF-*.md` atomically under
...
21. The editor webview now has an explicit high-contrast contract instead of relying on its dark
    ...
    existing white-on-purple action contrast without changing DGC's primary accent. Windows forced
    colors use system tokens, retain visible focus, distinguish diff additions/deletions by solid
    versus dashed rails, and keep failed/denied/resolved states shape- or text-identifiable. Tool
    cards expose `running`, `completed`, `failed`, `denied`, or `stopped` text to assistive technology,
    so their lifecycle never depends on a colored dot. Real Windows High Contrast, 200% zoom, and
    screen-reader passes remain manual release evidence rather than being inferred from static CSS.
22. The generated protocol-v4 JSON Schema is now an installed-package artifact rather than only a
    source-tree review file. `dgc protocol describe` emits the exact version, schema digest, byte and
    queue limits, required/optional command and event fields, and canonical slash catalogs without
    reading configuration, creating a session, checking for updates, or contacting a model. The
    `schema` route returns the byte-identical bundled contract; bounded `validate command|event`
    consumes file/stdin NDJSON, correlates results by physical line, fails the process on any bad
    frame, and never reflects hostile type/field values in diagnostics. Headless/ACP parsing and
...
24. The local editor candidate now closes the largest non-coding parity gaps without introducing a
    second agent implementation. Protocol v4 adds typed skill detail/reload, bundled documentation,
    live MCP server management, permission-rule management, durable memory, and session naming. The
    webview presents Skills, Docs, MCP, Permissions, Memory, and Hooks in dedicated searchable
    surfaces instead of serializing catalogs into the conversation, and routes the expanded canonical
    slash menu plus 27 palette commands through the same backend. MCP safe metadata is persisted in
    DGC configuration; extension-managed environment values and remote bearer tokens remain in VS
    Code SecretStorage and are injected only into a process-local environment (argv retains only an
    environment placeholder). Settings are grouped into General, Models, Agents, Security, and
    Extensions while retaining the existing black/mono/purple identity. Remote HTTP transport
    currently uses the explicit `mcp-remote` bridge and still
    requires Node/npm; native remote transport and real cross-platform assistive-technology review
    remain future evidence, not claimed completion.
