...
derived from `dgc.__version__`; editor metadata is derived from `editors/vscode/package.json` and
must agree with its package lock. The two channel versions do not need to be numerically equal.

Public GitHub (`OpenPeach-ai/dgc`) is the **product** only: CLI, extension, SDK, tests, release
scripts. `scripts/check-public-content.py` (preflight + CI job `public-tree`) fails if `site/`,
`site-src/`, `qa/site/`, `wrangler.json`, or website build scripts are tracked. vibedgc.com is
deployed from a local maintainer tree, not from a public git commit.
...
version. Ensure required CI and CodeQL checks are green and the source branch is clean. Commit the
   reviewed **product** sources as commit A and create annotated tag `vX.Y.Z` at A. Do not commit
   `site/` or `site-src/`.
2. At A, run `scripts/preflight.sh`, `scripts/build-release.sh`, and (when applicable)
   `scripts/release-extension.sh --build` (or reuse the verified captured precursor bundle).
   Preflight includes `scripts/check-public-content.py`, which rejects website paths in the git
   index. Optional local site deploy (Cloudflare) uses the untracked maintainer tree; it is not a
   public git commit.
3. From clean A, run `scripts/github-release.sh vX.Y.Z`. It verifies the tag matches `dgc.__version__`
   and HEAD, runs preflight, then atomically pushes A to public `main` and the tag. The tag workflow
   is the sole GitHub Release creator. It reruns product gates, builds the Python distribution and
