...
| --- | --- | --- | --- | --- |
| `dgc-sdk` (Python, PyPI) | **0.6.8** | CLI **0.44.1** | editor protocol v14 | Python 3.10+ |
| `@vibedgc/sdk` (Node, GitHub release tarball) | **0.6.8** | CLI **0.44.1** | editor protocol v14 | Node 22+ |
...
the CLI (0.44.1 or newer, protocol v14):
...
`__version__` (`"0.6.8"`), `PROTOCOL` (`14`), `REQUIRES_CLI` (`"0.44.1"`).
...
python3 -m pip download --no-deps dgc-sdk==0.6.8 -d dl
curl -fsSLO https://github.com/OpenPeach-ai/dgc/releases/download/sdk-v0.6.8/SHA256SUMS
(cd dl && sha256sum -c ../SHA256SUMS --ignore-missing)
gh attestation verify dl/dgc_sdk-0.6.8-py3-none-any.whl --repo OpenPeach-ai/dgc \
...
`pypi-attestations verify pypi --repository https://github.com/OpenPeach-ai/dgc pypi:dgc_sdk-0.6.8-py3-none-any.whl`
